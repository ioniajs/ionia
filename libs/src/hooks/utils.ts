...
/* tinycolor-mix */
tinycolor.mix = (color1: string, color2: string, amount: number) => {
...
function generateHoverColor(color: string, ratio = 5) {
	return tinycolor
		.mix('#ffffff', color, (currentEasing(ratio) * 100) / primaryEasing)
		.toHexString();
}

function generateActiveColor(color: string, ratio = 7) {
	return tinycolor
		.mix(
			'#333333',
			color,
...
function generateShadowColor(color: string, ratio = 9) {
	return tinycolor
		.mix(
			'#888888',
			color,
