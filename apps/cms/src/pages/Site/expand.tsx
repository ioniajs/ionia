...
import { Button, Form, Input, Switch, Select, Tooltip, Checkbox, Radio, Anchor, message } from 'antd';
				...
				>
					<BizSection title='内容配置' id='content'>
						...
						<Form.Item
							...
							initialValue={'0'}
						>
							<Radio.Group onChange={(e) => { setContentSignValue(e.target.value) }}>
								<Radio value='1'>开启</Radio>
								<Radio value='0'>不开启</Radio>
							</Radio.Group>
						</Form.Item>
						{contentSignValue === '1' && <>
							<Form.Item
								...
								rules={[{ required: contentSignValue === '1', message: '请输入新内容定义时间' }]}
							>
								<Input
									addonAfter={
										<Form.Item
							...
							<Form.Item name='contentSignType' label='新内容标记方式' initialValue={'1'}>
								<Radio.Group onChange={(e) => setContentSignTypeValue(e.target.value)}>
									<Radio value='1'>图片</Radio>
									<Radio value='2'>文字</Radio>
								</Radio.Group>
							</Form.Item>
							{contentSignTypeValue === '1' && <Form.Item
								name='contentSignPic'
								label='标记图片'
							...
							{contentSignTypeValue === '2' && <>
								<Form.Item
									...
									label='标记文字'
									rules={[{ required: true, message: '请输入新内容标记文字' }]}
								>
									<Input />
								</Form.Item>
								<Form.Item
									...
									label='文字颜色'
									rules={[{ required: true, message: '请选择新内容文字颜色' }]}
								>
									<ColorsPicker onChange={(colors) => { console.log(colors, 'ccccc') }} />
								</Form.Item>
							</>}
						</>}
						<Form.Item name='contentLikeLogin' label='点赞需要登录' initialValue={'0'}>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.contentLikeLogin)} />
						</Form.Item>
					</BizSection>
					<BizSection title='栏目配置' id='channel'>
						<Form.Item
							...
							initialValue={'0'}
						>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.channelNameRep)} />
						</Form.Item>
						<Form.Item
							name='channelPage'
							label='列表每页默认显示内容数'
					...
					<BizSection title='评论配置' id='comment'>
						...
						<Form.Item name='commentAudit' label='评论需要审核' initialValue={1}>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.commentAudit)} />
						</Form.Item>
						<Form.Item name='commentLink' label='允许输入链接' initialValue={0}>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.commentLink)} />
						</Form.Item>
					</BizSection>
					<BizSection title='邮件服务配置' id='mail-service'>
						...
						<Form.Item name='smtpSsl' label='使用SSL协议' initialValue={1}>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.smtpSsl)} />
						</Form.Item>
						<Form.Item
					...
					<BizSection title='水印配置' id='water-mark'>
						...
						{watermarkTypeValue === '1' && <Form.Item
							name='watermarkPic'
							label='水印图片'
					...
					<BizSection title='静态化服务配置' id='statics'>
						<Form.Item
							...
							initialValue={1}
						>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.staticServer)} />
						</Form.Item>
						<Form.Item
							name='staticPlatform'
							label='静态页发布平台'
						...
						<Form.Item
							...
							initialValue={'1'}
						>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.staticContent)} />
						</Form.Item>
						<Form.Item
							name='staticChannel'
							...
							initialValue={1}
						>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.staticChannel)} />
						</Form.Item>
						<Form.Item
							name='staticSize'
							label={
					...
					<BizSection title='站点访问配置' id='site-visit'>
						<Form.Item
							...
							initialValue={1}
						>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.siteLogin)} />
						</Form.Item>
						<Form.Item
							name='siteGray'
							label={
							...
							initialValue={0}
						>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.siteGray)} />
						</Form.Item>
						<Form.Item name='siteRelativePath' label='使用相对路径' initialValue={'0'}>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.siteRelativePath)} />
						</Form.Item>
					</BizSection>
					<BizSection title='网站群推送配置' id='site-push'>
						<Form.Item name='sitePush' label='允许网站群推送' initialValue={1}>
							<Switch checkedChildren='开启' unCheckedChildren='关闭' defaultChecked={!!Number(data?.data.sitePush)} />
						</Form.Item>
