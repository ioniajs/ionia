        ...
        <BizPage
            ...
            renderActions={() => {return (
                <>
                    <GobackButton />
                    <SaveButton />
                    <Button type='default' className='io-cms-site-save-expand__but'>保存并详细配置</Button>
                    <Button type='default' className='io-cms-site-save-expand__but'>保存并继续新建</Button>
                </>
            )}}
        >
            <BizForm form={form} renderActions={false} style={{ marginTop: '32px'}}>
                <Form.Item name='parentId' label='上级站点' rules={[{ required: true, message: '请选择上级站点'}]}>
                    <Cascader placeholder='请选择上级站点' />
                </Form.Item>
                <Form.Item name='name' label='站点名称' rules={[{ required: true, message: '请输入站点名称'}]}>
                    <Input placeholder='请输入站点名称' />
                </Form.Item>
                <Form.Item name='dir' label='站点目录' rules={[{ required: true, message: '请输入站点目录'}]}>
                ...
                <Form.Item name='desc' label='站点描述'>
                    <Input.TextArea placeholder='请输入站点描述' showCount={true} allowClear={true} maxLength={500} />
                </Form.Item>
                <Form.Item
                    name='status'
                    label={
                ...
                <Form.Item name='seoDesc' label='SEO描述'>
                    <Input.TextArea placeholder='请输入SEO描述' showCount={true} allowClear={true} maxLength={500} />
                </Form.Item>
            </BizForm>
        </BizPage> 
    )
