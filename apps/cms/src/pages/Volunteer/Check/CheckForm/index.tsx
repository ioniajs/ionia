...
export default ({ id, reloadData }: CheckFormProps) => {
    ...
    return (
        <BizModalForm
            ...
            title='志愿者审核'
            triggerRender={() => (
                <a onClick={() => { modalRef.current?.open() }}>审核</a>
            )}
            submitterRender={() => (
                <>
                    ...
                    <Button type='primary' onClick={() => {
                        form.validateFields().then(async values => {
                            ...
                            const param = {
                                ...
                                notPassReason: values.checkStatus === 3 ? values.notPassReason : '',
                            }
        ...
        >
            <Form form={form}>
                <Form.Item name='checkStatus' label='审核结果' rules={[{ required: true }]} initialValue={1}>
                    <Radio.Group onChange={(e) => setCheckStatusValue(e.target.value)}>
                        <Radio value={1}>通过</Radio>
                        <Radio value={3}>未通过</Radio>
                    </Radio.Group>
                </Form.Item>
                {checkStatusValue === 3 && <Form.Item name='notPassReason' label='未通过原因' rules={[{ required: true }]}>
