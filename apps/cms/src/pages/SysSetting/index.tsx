...
import { Button, Form, Select, Switch, Input, Tooltip } from 'antd';
import { PageContainer } from '@ant-design/pro-layout';
import { BizForm, BizPageModule, ImageUpload } from '@ionia/libs';
import { SysConfigDTO } from '@ionia/libs/src/services/infra/admin-system-setting'
import { getSystemSetting, saveSystemSetting } from '@ionia/libs/src/services/infra/admin-system-setting';
import './index.less';
...
export default () => {
    ...
    const handleSubmit = async (fields: Partial<SysConfigDTO>) => {
        console.log(fields, 'fffff')
        // const res = await saveSystemSetting({ ...fields });
        // return res;
    }

    // const { data, loading, run } = useRequest(() => getSystemSetting(id), {
    //     manual: true,
    //   });
    // 获取
    //   useMount(() => {
    ...
    return (
        <PageContainer pageHeaderRender={() => null} className='io-cms-setting_pagecontainer'>
            <ProCard className='io-cms-setting-container_card'>
                ...
                <BizPageModule title='系统风格设置'>
                    <BizForm form={form} renderActions={false}>
                        <Form.Item label='选择默认风格' name='defaultColor' initialValue='1' rules={[{ required: true, message: '请选择默认风格'}]}>
                            <Select>
                        ...
                        <Form.Item className='io-cms-form-item-tips' label={<span style={{ display: 'none'}}>不显示</span>} colon={false}>
                ...
                <BizPageModule title='文件上传设置'>
                    <BizForm form={form} renderActions={false}>
                        ...
                        <Form.Item name='maxPicSize' label='单张图片文件大小限制' rules={[
                            () => ({
                        ...
                        <Form.Item name='maxVideoSize' label='单个视频文件大小限制' rules={[
                            () => ({
                                validator(rule, value) {
                                    if (!value || /^[0-9]*$/g.test(value)) return Promise.resolve();
                                    return Promise.reject('请输入正整数');
                        ...
                        <Form.Item name='maxAudioSize' label='单个音频文件大小限制' rules={[
                            () => ({
                                validator(rule, value) {
                                    if (!value || /^[0-9]*$/g.test(value)) return Promise.resolve();
                                    return Promise.reject('请输入正整数');
                        ...
                        <Form.Item name='maxDocSize' label='单个文档大小限制' rules={[
                            () => ({
                                validator(rule, value) {
                                    if (!value || /^[0-9]*$/g.test(value)) return Promise.resolve();
                                    return Promise.reject('请输入正整数');
                        ...
                        <Form.Item name='attachCtlType' label='允许上传的附件类型' initialValue={1}>
                            <Select onSelect={(e: number) => { setAttachCtlTypeValue(e);console.log(e, 'eee'); }}>
                        ...
                        {!!attachCtlTypeValue && <Form.Item name='attachCtlTypes' label='请输入文件类型' rules={[{ required: true, message: '请输入文件类型'}]}>
                            <Input placeholder='多个文件类型用逗号隔开' />
                        </Form.Item>}
                        <Form.Item name='maxAttachSize' label='单个附件大小限制' rules={[
                            () => ({
                                validator(rule, value) {
                                    if (!value || /^[0-9]*$/g.test(value)) return Promise.resolve();
                                    return Promise.reject('请输入正整数');
