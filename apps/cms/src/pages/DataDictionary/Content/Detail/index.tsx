...
import { ProFormSelect, ProFormText, ProFormTextArea, ProFormSwitch, ProFormDigit } from '@ant-design/pro-form';
import { BizModalForm, BizModalFormRef, ImageUpload } from '@ionia/libs';
import { Button, Form, message } from 'antd';
import { dataDictionaryList, detailDataDictionary, DataDictionaryUpdateDTO, updateDataDictionary, DataDictionarySaveDTO, saveDataDiactionary } from '@ionia/libs/src/services';
...
export default ({ id, reloadTableData, source, typeId }: DetailFormProps) => {
    ...
    if (source === 'create') {
        triggerRender = <Button type='primary' onClick={() => { ref.current?.open(); setModalShow(true) }}>
            <i className='iconfont icon-plus1' />
    &nbsp;新建
</Button>
    } else if (source === 'edit') {
        triggerRender = <a onClick={() => { ref.current?.open(); setModalShow(true) }}>编辑</a>
    } else if (source === 'createSub') {
        triggerRender = <a onClick={() => { ref.current?.open(); setModalShow(true) }}>新增下级</a>
    }
    ...
    return (
        <BizModalForm
            ...
            onFinish={async (values) => {
                ...
                if (source === 'edit') { // 编辑详情
                    ...
                    const param = { ...values, typeId: typeId, parentId: source === 'createSub' ? id : '', pictureId: 0 }
        ...
        >
            <ProFormSelect name='parentId' label='上级字典数据' options={parentList} labelCol={{ span: 6 }}
                wrapperCol={{ span: 16 }} initialValue={source === 'createSub' ? typeId : ''} />
            <Form.Item name='pictureId' label='字典图'>
                <ImageUpload />
            </Form.Item>
            <ProFormText name='key' label='字典键值' disabled={source === 'edit'} fieldProps={{ maxLength: 30 }} rules={[{ required: true }]} placeholder='请输入键值' labelCol={{ span: 6 }}
                wrapperCol={{ span: 16 }} />
            <ProFormText name='label' label='字典标签' rules={[{ required: true }]} fieldProps={{ maxLength: 20 }} placeholder='请输入字典标签' labelCol={{ span: 6 }}
                wrapperCol={{ span: 16 }} />
            <ProFormDigit name='sortNum' label='排序值' rules={[{ required: true }]} placeholder='请输入排序值，值越大排序越靠前' labelCol={{ span: 6 }}
                wrapperCol={{ span: 16 }} min={1} fieldProps={{ precision: 0 }} max={99999999} initialValue={10} />
            <ProFormTextArea name='remark' label='备注' placeholder='请输入备注信息，仅限50字（选填）' fieldProps={{ showCount: true, maxLength: 50 }} labelCol={{ span: 6 }}
                wrapperCol={{ span: 16 }} />
            <ProFormSwitch name='status' label='状态' fieldProps={{ unCheckedChildren: '关闭', checkedChildren: '开启' }} rules={[{ required: true }]} initialValue={1} />
        </BizModalForm>
    )
}