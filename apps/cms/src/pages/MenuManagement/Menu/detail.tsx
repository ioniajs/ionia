...
import { BizPage, menuList, apiList, MenuDetail, MenuMod } from '@ionia/libs';
...
export default function MenuView({ match }: any) {
	let {
		params: { id },
	} = match;
	console.log(id);
	...
	const menuDeatil = useRequest(() => MenuDetail(id));
	...
	const onFinish = async (values: any) => {
		...
		const { code } = await MenuMod({ ...menuFormValues });
		if (code == 200) {
	...
	return (
		<>
			<BizPage breadcrumbs={[{ name: '菜单管理' }, { name: '详情' }]}>
